...
## 2026-09-15T10:44:15+04:00 — Task 0030 started

- Task: Organizer key for staff routes and a readiness check
- Agent/tool: Claude Code (Claude Opus 5) using FastAPI dependencies, `secrets.compare_digest`, pytest/httpx with dependency overrides, React hooks, Testing Library/Vitest, Playwright, curl against a keyed instance, and Git
- Prompt/reference: `docs/prompts/0030-organizer-key-and-readiness.md` and `docs/tasks/0030-organizer-key-and-readiness.md`; the user asked what a Google Cloud deployment would take and chose to start with access control and readiness
- Existing work: Tasks 0001–0029 committed and pushed; every route was open and `/health` never touched the database
- Decisions: One shared secret rather than accounts (decision 0003 revised), unset by default so local development, tests, and the browser proofs are unchanged; header plus a query-parameter form only for the stream because `EventSource` cannot send headers; constant-time comparison; the frontend prompts on 401 and remembers the key in `localStorage`; a separate `/ready` so liveness and readiness stay distinct
- Status: Completed at 2026-09-15T10:55:45+04:00
- Result: With `ORGANIZER_KEY` set, event creation, rescheduling, statistics, the live stream, and check-in require the key while participant routes stay open; the organizer and check-in screens ask once and continue working; `/ready` reports database availability.
- Failures diagnosed: The readiness test's unreachable engine hung on Windows without a connect timeout (the run was moved to the background and stopped); a one-second `connect_timeout` fixed it. An async fixture needed `pytest_asyncio.fixture`. Oxlint flagged the hook living in a component file and an effect missing a dependency; the hook moved to `src/hooks/` with `useCallback`-stable functions so the dashboard effect can depend on them without reconnecting every render. A scripted edit failed because `src/hooks/` did not exist yet.
- Tests/checks: Ruff lint/format; backend pytest (104 passed, four new key/readiness tests); Oxlint; Vitest (23 passed, two new); TypeScript/Vite build; Playwright (2 passed, unkeyed, against the running servers); live keyed instance on port 8001: `/ready` ready, create 401 without key / 201 with key, public list 200, check-in 401 without key; port released; Git whitespace check.
- Self-review: All five acceptance criteria are met; no domain logic changed; the key never appears in email or in participant screens; the query-parameter form is limited to the stream and documented. No Critical or Important findings remain.

## 2026-09-15T10:56:29+04:00 — Task 0031 started

- Task: Containerised single-host deployment
- Agent/tool: Claude Code (Claude Opus 5) using Docker multi-stage builds, uv in containers, nginx, Caddy 2 (automatic HTTPS, basic_auth, unbuffered reverse proxy), Docker Compose overlays with `!override` and service-condition dependencies, curl and a real browser through the proxy, and Git
- Prompt/reference: `docs/prompts/0031-containerised-single-host-deployment.md` and `docs/tasks/0031-containerised-single-host-deployment.md`
- Existing work: Task 0030 committed; Compose provided only PostgreSQL and Mailpit and the applications ran from developer tooling
- Decisions: Keep the base Compose file and local workflow untouched and add a deploy overlay; one backend image for API, worker, and a one-shot migration service ordered by health/completion conditions; same-origin frontend (`VITE_API_BASE_URL=""`) behind Caddy so CORS is not a deployment concern; Mailpit stays the mail sink and its inbox is published under `/mail` with basic auth as part of the demo; the Google Cloud VM commands are documented as standard steps and explicitly marked as not executed in this task
- Status: Completed at 2026-09-15T11:05:12+04:00
- Result: `docker compose --env-file .env.deploy -f docker-compose.yml -f docker-compose.deploy.yml up -d --build` brings up the whole product with HTTPS on one host; a full local rehearsal through the proxy passed and the guide records it.
