...
4. Run the notification worker and open Mailpit at `http://localhost:8025`: `first@example.com` has a confirmation with a ticket and `second@example.com` a waiting-list email with place 1; both end with a self-service link. Open the first participant's link (or use **Cancel participation** on the result page), decline the confirmation once to show that nothing changes, then choose it again and confirm. Show **Your participation is cancelled**, the message that the previous ticket is no longer active, and the absence of an active ticket link.
