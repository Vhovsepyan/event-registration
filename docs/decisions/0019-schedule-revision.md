...
Every actual change creates one new reschedule row per active participant, including repeated transitions and returns to earlier dates. No-op updates remain idempotent. Since task 0025, when a newer notice is queued for a recipient any older notice that is still `PENDING` is marked `SUPPRESSED` (`superseded by schedule revision N`): the intent of every change stays recorded, while the participant receives the latest schedule once instead of a stale intermediate one. Historical rows keep their original dedupe keys; the migration only adds the revision column with default 0. Task 0020 uses the persisted revision to decide at dispatch whether a queued reminder still describes the current schedule.
